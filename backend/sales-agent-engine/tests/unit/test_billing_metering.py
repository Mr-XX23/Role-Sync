...
async def test_every_completed_model_call_is_charged_once_to_the_workspace_in_scope():
    ...
    router = ModelRouter({"gemini": gemini}, RULES, NoopTracingClient(), credit_meter=meter)
...
async def test_gemini_usage_counts_thinking_as_output_and_reports_the_cached_prompt():
    ...
    billing = FakeBillingService()
    meter, client = _metered(billing)
    router = ModelRouter({"gemini": _gemini(chunks)}, RULES, NoopTracingClient(), credit_meter=meter)
...
async def test_openrouter_usage_reports_the_cached_prompt_and_the_model_that_answered():
    ...
    billing = FakeBillingService()
    meter, client = _metered(billing)
    router = ModelRouter({"openrouter": OpenRouterProvider(api_key="k", http=http)}, RULES, NoopTracingClient(), credit_meter=meter)
...
async def test_on_failover_attempts_that_used_tokens_are_charged_and_attempts_that_did_not_are_not():
    ...
    refused = ModelRouter({"gemini": Brain("gemini", fail=RateLimited("429")), "openrouter": backup}, RULES, NoopTracingClient(), credit_meter=meter)
    ...
    mid_answer = ModelRouter(
        {"gemini": Brain("gemini", fail=cut_off, after_text=True), "openrouter": backup}, RULES, NoopTracingClient(), credit_meter=meter
    )
    with usage_scope(WORKSPACE, USER, reference=str(SESSION)):
...
async def test_a_gemini_answer_spent_entirely_on_thinking_is_charged_though_it_fails_over():
    ...
    router = ModelRouter({"gemini": _gemini(chunks), "openrouter": backup}, RULES, NoopTracingClient(), credit_meter=meter)
...
async def test_a_failure_without_reported_usage_or_with_no_fallback_is_handled_without_extra_charges():
    billing = FakeBillingService()
    meter, client = _metered(billing)
    router = ModelRouter({"openrouter": Brain("openrouter", fail=RateLimited("429"))}, RULES, NoopTracingClient(), credit_meter=meter)
...
async def test_model_calls_with_no_workspace_in_scope_are_not_charged():
    billing = FakeBillingService()
    meter, client = _metered(billing)
    router = ModelRouter({"gemini": Brain("gemini", usage=Usage(1_000, 100))}, RULES, NoopTracingClient(), credit_meter=meter)
...
async def test_metering_problems_never_fail_the_model_call():
    ...
    router = ModelRouter({"gemini": Brain("gemini", usage=Usage(1_000, 100))}, RULES, NoopTracingClient(), credit_meter=BrokenMeter())
...
async def test_a_grounded_web_answer_is_charged_as_a_search_with_its_tokens_and_one_grounded_prompt(model, unit):
    ...
    router = ModelRouter({"gemini": grounding}, rules, NoopTracingClient(), credit_meter=meter)
...
async def test_each_tavily_search_is_charged_and_a_failed_one_is_not():
    ...
    router = ModelRouter({}, RULES, NoopTracingClient(), credit_meter=meter)
...
async def test_an_ai_skill_draft_is_charged_to_the_drafting_reps_workspace():
    ...
    billing = FakeBillingService()
    meter, client = _metered(billing)
    router = ModelRouter({"gemini": Brain("gemini", usage=Usage(2_400, 700), text=text)}, RULES, NoopTracingClient(), credit_meter=meter)
