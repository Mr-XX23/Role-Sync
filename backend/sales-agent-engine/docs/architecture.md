...
- **Write tools (Phase 3, decided 2026-09-12).** Calendar events, Slack messages and Notion pages go through Composio. Generated documents (docx, pptx, xlsx, pdf, md, rendered in the engine) and quotes are saved to the rep's **Google Drive** (Composio file upload); if Drive isn't connected or is full they go to the **workspace knowledge vault** instead (data-pipeline now also accepts PPTX and XLSX uploads), and the chat links to the file. Catalog writes use data-pipeline's catalog API: create items (with options and SKUs), update details, discount limits and each SKU's price, currency, barcode, weight or status, record stock movements, reserve and release stock, and retire (never hard-delete). Since 2026-09-14 the coordinator also sets up the catalog (CATALOG_SETUP scope): categories and stock locations (removed only while unused: a location that ever held stock keeps its history), SKUs added to existing items, and retired items restored; reads list and filter items, show one item with its options and stock per location, describe the catalog's setup and list reservations. Since data-pipeline's typed stock movements (2026-09-13) the ledger is append-only: the agent records what happened (received, sold, shipped, damaged, lost, returned), and only workspace OWNER/ADMIN may replace a count, with a note and the on-hand figure it was based on (409 if stock moved since); the engine checks the role before asking for approval. Viewers of a workspace can't change its catalog. Quotes price SKUs from the catalog, enforce each item's `max_discount_pct`, and can reserve the quoted stock.
...
- **Connected apps (PR 5, decided 2026-09-14/15).** What a rep syncs from Gmail, Calendar, Slack, Drive and Notion is private to them (data-pipeline `connector_privacy.py`). The agent manages the rep's own connections through data-pipeline's connector API: `list_connected_apps` and `get_app_sync_history` read; `sync_app_now`, `change_app_sync_settings`, `set_app_auto_sync` and `disconnect_app` are coordinator-only (CONNECTORS scope) writes with approval. Every write first checks that Composio lists the rep's account as ACTIVE and that data-pipeline's connection is set up and idle, because a sync of an app that isn't really connected ends as if everything were synced and skips its history. Settings are sent whole (the config route resets unsent fields, switching auto-sync off) and saving starts a sync. Disconnecting removes the account in every workspace and stops the agent's own tools for that app, so it is irreversible here. Connecting (a browser OAuth flow), resync, retry-failed (a full resync underneath), webhooks and purging stay on the Connectors page; the agent offers auto-sync every 30 minutes at most.
- **The rep's profile (PR 4, decided 2026-09-14).** workspace-service allows **24 profile saves** and, counted separately, **3 profile photo changes** every 24 hours (`ProfileChangeLimits`; each window opens with its first change). Only a different picture counts as a photo change (keeping or removing the photo doesn't), a save that brings a new photo counts against both and is refused whole when either is used up, and the upload endpoints refuse before anything reaches Cloudinary. `GET /api/v1/workspaces/profile/limits` reports both. The prompt's "about the rep" block carries their name, title, department, company, location, language (unless English), communication style, their own words for the agent, expertise, skills and bio; contact details and links stay out of prompts and are read with `get_my_profile`. The time zone from the rep's settings is used only when a request carries no browser time zone (UTC, which every profile starts with, is ignored). `update_my_profile` checks the saves left before the rep is asked and puts every change into one save; the photo is changed on the Profile page only. The cached profile is dropped after the agent changes it.
...
- **Skills (decided 2026-09-14).** A skill is a playbook for one recurring sales job, kept in the engine (`app/skills`, tables `agent.skill`, `skill_version`, `skill_setting`, `skill_usage`). Ten built-in SKILL.md files ship with the code; owners and admins write workspace skills and can customize a built-in for the workspace (and reset it); members and above write private skills only their own agent uses; viewers read. Every skill is on until switched off: each rep switches skills for their own agent, and an owner or admin can switch a built-in or workspace skill off for everyone (at most 30 on per rep). How the agent uses them: the planner's instructions list the rep's switched-on skills (id, name, when to use it), and the model loads the one a request matches with `use_skill`, a READ tool every agent has (no approval, audited). A skill the rep picked for a request (`POST /chat` `skill`) is loaded before the model's first step, as a `use_skill` call through the gate, so it is audited and shown in the transcript like any call; `delegate` can hand a skill to a sub-agent, which loads it the same way before its first step. Skill text reaches the model as a tool result framed as guidance under its fixed rules: skills never grant tools or scopes, and actions still pause for approval. Every save is a version (optimistic: a stale `expected_version` is a 409), archiving and resetting are recoverable, SKILL.md files come in through a preview (instructions only; tools this agent doesn't have are dropped), and AI drafts are one model call each, limited per rep per day. Each load is counted as chosen by the agent, picked by the rep, or handed to a sub-agent.
